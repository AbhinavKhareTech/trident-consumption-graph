...
class MockFoodMCP(MCPServer):
    ...
    async def call_tool(self, tool_name: str, params: dict[str, Any]) -> MCPToolResult:
        if tool_name == "search_restaurants":
            ...
            menu = self._menus.get(rid, [{"name": "Biryani", "price": 350}, {"name": "Naan", "price": 60}])
            return MCPToolResult(tool_name=tool_name, success=True, data={"menu": menu})
        elif tool_name == "update_food_cart":
            items = params.get("items", [])
            self._cart["items"].extend(items)
            self._cart["total"] = sum(i.get("price", 0) for i in self._cart["items"])
            ...
            return MCPToolResult(tool_name=tool_name, success=True, data={"status": "preparing", "eta_minutes": 30})
    ...
    def available_tools(self) -> list[str]:
        return ["search_restaurants", "get_restaurant_menu", "update_food_cart", "place_food_order", "track_food_order"]
